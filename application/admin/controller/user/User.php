    ...
    public function index()
    {
       ...
       if($group_ids[0]>=7){
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $row) {
                $row->visible(['id','bianhao','username','nickname','mobile','email','usdt','usdt_dj','sfz_fimage','sfz_bimage','sfz_pimage','createtime','status','sfz_status','diqu','pay_status','agent_group_id','invite','agent_group_id',"trust","letstalk","cz_switch","rj_switch","cj_switch","yx_switch"]);
                $row->visible(['group']);
