...
class Sell extends Api
{
    ...
    public function list(){
        ...
        $list = $orderCash->field('orderid,user_id,pay_type,bi_type,act_num,amount,rate,diqu')->where(['status'=>1,'pay_status'=>0,'diqu'=>$diqu])->page($page)->select();
    ...
    public function preorder(){ 
        ...
        $res = $orderCash->update(['pay_status'=>1,'receiveid'=>$receiveid,'pintai_id'=>$this->access_key],['id'=>$info['id']]);
        if($res){
    ...
    */   
    public function payOrder(){ 
        ...
        $seller_name = $this->request->param('seller_name','');     // 收款人名称
        $pinzheng_image = $this->request->param('pinzheng_image');  // 凭证图片


        if(!$orderid || !$pay_type || $receiveid || $seller_name){
          return  $this->error('参数错误');
        }

        if(!$backurl){
            return $this->error('回调地址未填写');
        }
