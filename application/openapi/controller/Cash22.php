    ...
    { 
        ...
        if($this->access_key == '1250730111'){
            ...
            if($params['amount']<=100000) {
                if($cacheData){
                    ...
                    if(!$userInfo){
                        ...
                        if($params['amount']>=7000){
                            ...
                            $ptulist = $userModel->where($where)->where('usdt',">=",$usdt)->where("trust",2)->where("min_cny",">=",7000)->where("max_cny","<=",210000)->order($order)->column('id'); //普通用户
                ...
                $where['big'] = 1;
                $order = 'pay_sort desc,id desc';
                $ulistIds = $userModel->where($where)->where("min_cny",">=",100000)->where("max_cny","<=",510000)->order($order)->column('id');
