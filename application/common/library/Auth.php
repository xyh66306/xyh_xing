...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username','paypwd', 'nickname', 'mobile', 'avatar', 'usdt','usdt_dj', 'email', 'invite', 'status','group_id','sfz_status','diqu','letstalk','cz_switch'];
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        if($extend['invite'] && !User::get($extend['invite'])) {
            $this->setError('邀请人不存在');
            return false;
        }
    ...
    public function checkpaypwd($paypwd){
        
        $jm_pwd = $this->getEncryptPassword($paypwd, $this->_user->salt);
        if($jm_pwd == $this->_user->paypwd){
            return true;
        }else{
            return false;
        }
    ...
    public function update_sfz($user_id,$sfz_f, $sfz_b,$sfz_p)
    {
        ...
        Db::startTrans();
        try {
            User::update(['sfz_fimage'=>$sfz_f,'sfz_bimage'=>$sfz_b,'sfz_pimage'=>$sfz_p],['id'=>$user_id]);          
            Db::commit();
        } catch (Exception $e) {
            Db::rollback();
            $this->setError($e->getMessage());
