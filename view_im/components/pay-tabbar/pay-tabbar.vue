<template>
	<view class="zdTabBar">
		<view class="ul df" :style="{'padding-bottom': system.pdbottom}">
...
export default {
	...
	mounted() {
		...
		if (safeArea) {
			// #ifdef MP-WEIXIN
			this.system.pdbottom = `${screenHeight - safeArea.bottom}rpx`
			// #endif
			// #ifndef MP-WEIXIN
			this.system.pdbottom =  `${safeAreaInsets.bottom}rpx`
			// #endif
		} else {
			this.system.pdbottom =  0
		}
		let front_menu = uni.getStorageSync('front_menu');
